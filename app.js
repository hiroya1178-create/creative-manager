...
function load(key, fallback){ try{ const raw = localStorage.getItem("cm-public-v23-" + key); return raw ? JSON.parse(raw) : fallback; }catch(e){ return fallback; } }
function persist(){
  localStorage.setItem("cm-public-v23-orders", JSON.stringify(state.orders));
  localStorage.setItem("cm-public-v23-staff", JSON.stringify(state.staff));
  localStorage.setItem("cm-public-v23-templates", JSON.stringify(state.templates));
}
...
function outsourceStatusClass(status){ if(status==="依頼前") return ""; if(status==="依頼済み") return "internal"; if(status==="制作中") return "outsource"; if(status==="確認中") return "received"; return "ok"; }
...
function openCreate(prefillTemplateId=null){
  ...
  if(prefillTemplateId){
    ...
    if(tpl){
      state.createForm = { ...defaultCreateForm(), projectName: tpl.title, amount: String(tpl.price || 80000) };
    }
  } else {
...
function closeCreate(){ state.createOpen = false; state.createErrors = {}; state.prefillTemplateId = null; render(); }

function renderNav(){
  const items = [["dashboard","ダッシュボード"],["orders","受注管理"],["staff","スタッフ管理"],["templates","テンプレート管理"],["calendar","日程カレンダー"],["outsource","外注管理"]];
  return `<aside class="sidebar"><div class="brand">デザインマネージャー</div><div class="brand-sub">クリエイティブ管理</div><div class="nav">${items.map(([k,l])=>`<button class="${state.currentPage===k?'active':''}" onclick="setPage('${k}')">${l}</button>`).join("")}</div><div class="version">デザインマネージャー 公開版 v2.3</div></aside>`;
}
...
function renderCalendar(){
  ...
  <div class="card"><div style="font-weight:700;margin-bottom:14px">日付クリック詳細</div>${!state.selectedCalendarDate?`<div class="muted">カレンダーの日付をクリックすると、その日の案件が表示されます。</div>`: selectedOrders.length===0?`<div class="muted">${new Date(state.selectedCalendarDate).toLocaleDateString('ja-JP')} の案件はありません。</div>`: selectedOrders.map(o=>`<div class="detail-item"><div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;flex-wrap:wrap"><div><div style="font-weight:700">${esc(o.projectName)}</div><div class="muted" style="margin-top:6px">顧客: ${esc(o.client)} / 担当: ${esc(o.assignee)}</div></div><button class="btn primary" onclick="openCalendarQuickEdit(${o.id})">この案件を編集</button></div><div class="badges" style="margin-top:10px"><span class="badge ${statusClass(o.status)}">${esc(o.status)}</span><span class="badge ${judgeClass(o.judge)}">${esc(o.judge)}</span></div></div>`).join("")}</div></div>`;
}
function renderOutsource(){
  ...
  <div class="stat-grid" style="grid-template-columns:repeat(4,minmax(0,1fr))">${statCard("外注候補件数", outs.length, "")}${statCard("外注候補合計金額", formatYen(total), "")}${statCard("受信済み件数", receivedCount, "")}${statCard("依頼済み件数", requestedCount, "")}</div>
  <div style="margin-top:16px">${outs.length===0?`<div class="card muted">現在、外注候補案件はありません。</div>`: outs.map(o=>`<div class="card" style="margin-bottom:14px"><div style="display:flex;justify-content:space-between;gap:14px;flex-wrap:wrap"><div><div style="font-size:18px;font-weight:800">${esc(o.projectName)}</div><div class="muted" style="margin-top:6px">顧客: ${esc(o.client)} / 担当: ${esc(o.assignee)}</div><div class="muted" style="margin-top:6px">金額: ${formatYen(o.amount)} / 完了予定: ${esc(o.finishDate||"未設定")}</div><div class="muted" style="margin-top:6px">外注先: ${esc(o.outsourceVendor||"未設定")} / 受信日: ${esc(o.receivedDate||"未設定")}</div></div><div class="badges"><span class="badge ${statusClass(o.status)}">${esc(o.status)}</span><span class="badge ${judgeClass(o.judge)}">${esc(o.judge)}</span><span class="badge ${outsourceStatusClass(o.outsourceStatus||"依頼前")}">${esc(o.outsourceStatus||"依頼前")}</span></div></div>${o.outsourceMemo?`<div class="detail-item" style="margin-top:12px">${esc(o.outsourceMemo)}</div>`:""}<div class="inline-actions"><button class="btn">指示書を見る</button><button class="btn" onclick="openOutsourceEditor(${o.id})">外注情報を編集</button><button class="btn primary" onclick="markReceived(${o.id})">納品受信にする</button></div></div>`).join("")}</div>`;
}
...
function renderOrderModal(){
  ...
  <div class="inline-actions"><button class="btn primary" onclick="saveSelected()">保存する</button><button class="btn" onclick="duplicateSelected()">複製する</button><button class="btn danger" onclick="deleteSelected()">削除する</button><button class="btn" onclick="closeOrder()">閉じる</button></div></div>
...
function renderCreateModal(){
  ...
  return `<div class="modal-backdrop" onclick="if(event.target===this) closeCreate()"><div class="modal" style="width:min(820px,100%)"><div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;margin-bottom:16px"><div><h3>新規案件追加</h3><p class="sub">顧客名と案件名を先に入れると進めやすいです。PDFを添付すると候補を自動入力します。</p></div><button class="btn" onclick="closeCreate()">閉じる</button></div>
...
function renderStaffEditorModal(){
  if(!state.staffEditorOpen) return "";
  const member = state.staff.find(s => s.id === state.editingStaffId) || { name:"", role:"デザイナー", email:"", hoursPerDay:8, skills:[""] };
  return `<div class="modal-backdrop" onclick="if(event.target===this) closeStaffEditor()"><div class="modal" style="width:min(820px,100%)"><div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;margin-bottom:16px"><div><h3>スタッフ編集</h3><p class="sub">スタッフ情報を追加・編集できます</p></div><button class="btn" onclick="closeStaffEditor()">閉じる</button></div>
  <div class="form-grid">
    <div><div class="help" style="margin-bottom:6px">名前</div><input value="${esc(member.name||"")}" oninput="patchStaffEditor('name', this.value)"></div>
    <div><div class="help" style="margin-bottom:6px">役職</div><input value="${esc(member.role||"")}" oninput="patchStaffEditor('role', this.value)"></div>
    <div><div class="help" style="margin-bottom:6px">メール</div><input value="${esc(member.email||"")}" oninput="patchStaffEditor('email', this.value)"></div>
    <div><div class="help" style="margin-bottom:6px">1日稼働時間</div><input type="number" value="${esc(member.hoursPerDay||8)}" oninput="patchStaffEditor('hoursPerDay', this.value)"></div>
    <div style="grid-column:1/-1"><div class="help" style="margin-bottom:6px">スキル（1行に1つ）</div><textarea oninput="patchStaffEditorSkills(this.value)">${esc((member.skills||[]).join('\n'))}</textarea></div>
  </div>
  <div class="top-actions" style="justify-content:flex-end;margin-top:16px"><button class="btn" onclick="closeStaffEditor()">キャンセル</button><button class="btn primary" onclick="saveStaffEditor()">保存する</button></div></div></div>`;
}
function renderTemplateEditorModal(){
  if(!state.templateEditorOpen) return "";
  const tpl = state.templates.find(t => t.id === state.editingTemplateId) || { title:"", category:"ロゴデザイン", price:80000, hours:12, tasks:[""] };
  return `<div class="modal-backdrop" onclick="if(event.target===this) closeTemplateEditor()"><div class="modal" style="width:min(820px,100%)"><div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;margin-bottom:16px"><div><h3>テンプレート編集</h3><p class="sub">テンプレート情報を追加・編集できます</p></div><button class="btn" onclick="closeTemplateEditor()">閉じる</button></div>
  <div class="form-grid">
    <div><div class="help" style="margin-bottom:6px">テンプレート名</div><input value="${esc(tpl.title||"")}" oninput="patchTemplateEditor('title', this.value)"></div>
    <div><div class="help" style="margin-bottom:6px">カテゴリ</div><select onchange="patchTemplateEditor('category', this.value)">${["ロゴデザイン","ウェブデザイン","パッケージ","広告デザイン"].map(c=>`<option ${tpl.category===c?'selected':''}>${c}</option>`).join("")}</select></div>
    <div><div class="help" style="margin-bottom:6px">価格</div><input type="number" value="${esc(tpl.price||0)}" oninput="patchTemplateEditor('price', this.value)"></div>
    <div><div class="help" style="margin-bottom:6px">標準工数</div><input type="number" value="${esc(tpl.hours||0)}" oninput="patchTemplateEditor('hours', this.value)"></div>
    <div style="grid-column:1/-1"><div class="help" style="margin-bottom:6px">作業項目（1行に1つ）</div><textarea oninput="patchTemplateEditorTasks(this.value)">${esc((tpl.tasks||[]).join('\n'))}</textarea></div>
  </div>
  <div class="top-actions" style="justify-content:flex-end;margin-top:16px"><button class="btn" onclick="closeTemplateEditor()">キャンセル</button><button class="btn primary" onclick="saveTemplateEditor()">保存する</button></div></div></div>`;
}
function renderOutsourceEditorModal(){
  if(!state.outsourceEditorId) return "";
  const order = state.orders.find(o => o.id === state.outsourceEditorId);
  if(!order) return "";
  return `<div class="modal-backdrop" onclick="if(event.target===this) closeOutsourceEditor()"><div class="modal" style="width:min(820px,100%)"><div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;margin-bottom:16px"><div><h3>外注案件編集</h3><p class="sub">外注先・依頼状況・受信日を管理できます</p></div><button class="btn" onclick="closeOutsourceEditor()">閉じる</button></div>
  <div class="form-grid">
    ...
    <div><div class="help" style="margin-bottom:6px">外注先</div><input value="${esc(order.outsourceVendor||"")}" oninput="patchOutsourceEditor('outsourceVendor', this.value)"></div>
    <div><div class="help" style="margin-bottom:6px">依頼状況</div><select onchange="patchOutsourceEditor('outsourceStatus', this.value)">${["依頼前","依頼済み","制作中","確認中","納品受信"].map(s=>`<option ${(order.outsourceStatus||"依頼前")===s?'selected':''}>${s}</option>`).join("")}</select></div>
    <div><div class="help" style="margin-bottom:6px">受信日</div><input type="date" value="${esc(order.receivedDate||"")}" oninput="patchOutsourceEditor('receivedDate', this.value)"></div>
    <div><div class="help" style="margin-bottom:6px">完了予定日</div><input type="date" value="${esc(order.finishDate||"")}" oninput="patchOutsourceEditor('finishDate', this.value)"></div>
    <div style="grid-column:1/-1"><div class="help" style="margin-bottom:6px">外注メモ</div><textarea oninput="patchOutsourceEditor('outsourceMemo', this.value)">${esc(order.outsourceMemo||"")}</textarea></div>
  </div>
  <div class="top-actions" style="justify-content:flex-end;margin-top:16px"><button class="btn" onclick="closeOutsourceEditor()">キャンセル</button><button class="btn primary" onclick="saveOutsourceEditor()">保存する</button></div></div></div>`;
}
function renderCalendarQuickEditModal(){
  if(!state.calendarQuickEditId) return "";
  const order = state.orders.find(o => o.id === state.calendarQuickEditId);
  if(!order) return "";
  return `<div class="modal-backdrop" onclick="if(event.target===this) closeCalendarQuickEdit()"><div class="modal" style="width:min(820px,100%)"><div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;margin-bottom:16px"><div><h3>日程から案件編集</h3><p class="sub">担当者・完了予定日・ステータスをその場で変更できます</p></div><button class="btn" onclick="closeCalendarQuickEdit()">閉じる</button></div>
  <div class="form-grid">
    <div><div class="help" style="margin-bottom:6px">案件名</div><input value="${esc(order.projectName)}" readonly></div>
    <div><div class="help" style="margin-bottom:6px">顧客名</div><input value="${esc(order.client)}" readonly></div>
    <div><div class="help" style="margin-bottom:6px">担当者</div><select onchange="patchCalendarQuickEdit('assignee', this.value)">${staffOptions.map(n=>`<option ${order.assignee===n?'selected':''}>${n}</option>`).join("")}</select></div>
    <div><div class="help" style="margin-bottom:6px">完了予定日</div><input type="date" value="${esc(order.finishDate||"")}" oninput="patchCalendarQuickEdit('finishDate', this.value)"></div>
    <div style="grid-column:1/-1"><div class="help" style="margin-bottom:6px">ステータス</div><select class="${statusClass(order.status)}" onchange="patchCalendarQuickEdit('status', this.value)">${["納期OK","納期NG","納品受信"].map(s=>`<option ${order.status===s?'selected':''}>${s}</option>`).join("")}</select></div>
  </div>
  <div class="notice-box" style="margin-top:16px"><div style="font-weight:700;margin-bottom:8px">変更後の通知文面プレビュー</div>${esc(buildNotice(order))}</div>
  <div class="top-actions" style="justify-content:flex-end;margin-top:16px"><button class="btn" onclick="closeCalendarQuickEdit()">キャンセル</button><button class="btn primary" onclick="saveCalendarQuickEdit()">保存する</button></div></div></div>`;
}
...
function saveSelected(){
  ...
  order.judge = statusToJudge(order.status); order.notice = buildNotice(order); order.history = [...(order.history||[]), "案件情報を保存しました"]; persist(); render();
}
...
function validateCreate(){ const e = {}; const f = state.createForm; if(!f.client.trim()) e.client="顧客名は必須です"; if(!f.projectName.trim()) e.projectName="案件名は必須です"; if(!String(f.amount).trim() || Number(f.amount)<=0) e.amount="金額を正しく入力してください"; if(!f.finishDate) e.finishDate="完了予定日は必須です"; state.createErrors=e; return Object.keys(e).length===0; }
function createOrder(){
  ...
  state.createForm = defaultCreateForm(); state.createErrors={}; state.createOpen=false; state.currentPage="orders"; persist(); render();
}
...
function markReceived(id){
  state.orders = state.orders.map(o=>{ if(o.id!==id) return o; const next = {...o,status:"納品受信",judge:"納品受信",outsourceStatus:"納品受信",receivedDate:new Date().toISOString().slice(0,10),history:[...(o.history||[]),"外注案件を納品受信に変更"]}; next.notice = buildNotice(next); return next; });
  persist(); render();
}
...
async function handlePdfUpload(event){
  ...
  try{
    ...
    state.createForm = { ...state.createForm, client: fields.client || state.createForm.client, projectName: fields.projectName || state.createForm.projectName, amount: fields.amount || state.createForm.amount, finishDate: fields.finishDate || state.createForm.finishDate, extractedText: fields.extractedText || "", sourceFileName: file.name };
    state.createErrors = {};
  }catch(err){
...
// staff editor
function openStaffEditor(id=null){ state.editingStaffId = id; state.staffEditorOpen = true; render(); }
function closeStaffEditor(){ state.staffEditorOpen = false; state.editingStaffId = null; render(); }
function patchStaffEditor(key,val){
  if(state.editingStaffId){
    state.staff = state.staff.map(s => s.id===state.editingStaffId ? {...s,[key]: key==="hoursPerDay" ? Number(val||0) : val} : s);
  } else {
    state._newStaff = state._newStaff || { id:null, name:"", role:"デザイナー", email:"", hoursPerDay:8, skills:[""] };
    state._newStaff[key] = key==="hoursPerDay" ? Number(val||0) : val;
  }
  render();
}
function patchStaffEditorSkills(text){
  if(state.editingStaffId){
    state.staff = state.staff.map(s => s.id===state.editingStaffId ? {...s, skills:text.split("\n")} : s);
  } else {
    state._newStaff = state._newStaff || { id:null, name:"", role:"デザイナー", email:"", hoursPerDay:8, skills:[""] };
    state._newStaff.skills = text.split("\n");
  }
  render();
}
function saveStaffEditor(){
  let payload;
  if(state.editingStaffId){
    payload = state.staff.find(s => s.id===state.editingStaffId);
    payload.skills = (payload.skills||[]).map(s=>String(s).trim()).filter(Boolean);
  } else {
    payload = state._newStaff || { name:"", role:"デザイナー", email:"", hoursPerDay:8, skills:[""] };
    if(!payload.name || !payload.name.trim()) return;
    payload = { ...payload, id: Date.now(), skills:(payload.skills||[]).map(s=>String(s).trim()).filter(Boolean) };
    state.staff = [payload, ...state.staff];
    delete state._newStaff;
  }
  persist(); closeStaffEditor();
}
function deleteStaff(id){ state.staff = state.staff.filter(s => s.id !== id); persist(); render(); }

// template editor
function openTemplateEditor(id=null){ state.editingTemplateId = id; state.templateEditorOpen = true; render(); }
function closeTemplateEditor(){ state.templateEditorOpen = false; state.editingTemplateId = null; render(); }
function patchTemplateEditor(key,val){
  if(state.editingTemplateId){
    state.templates = state.templates.map(t => t.id===state.editingTemplateId ? {...t,[key]: (key==="price"||key==="hours")?Number(val||0):val} : t);
  } else {
    state._newTemplate = state._newTemplate || { title:"", category:"ロゴデザイン", price:80000, hours:12, tasks:[""] };
    state._newTemplate[key] = (key==="price"||key==="hours") ? Number(val||0) : val;
  }
  render();
}
function patchTemplateEditorTasks(text){
  if(state.editingTemplateId){
    state.templates = state.templates.map(t => t.id===state.editingTemplateId ? {...t, tasks:text.split("\n")} : t);
  } else {
    state._newTemplate = state._newTemplate || { title:"", category:"ロゴデザイン", price:80000, hours:12, tasks:[""] };
    state._newTemplate.tasks = text.split("\n");
  }
  render();
}
function saveTemplateEditor(){
  let payload;
  if(state.editingTemplateId){
    payload = state.templates.find(t => t.id===state.editingTemplateId);
    payload.tasks = (payload.tasks||[]).map(t=>String(t).trim()).filter(Boolean);
  } else {
    payload = state._newTemplate || { title:"", category:"ロゴデザイン", price:80000, hours:12, tasks:[""] };
    if(!payload.title || !payload.title.trim()) return;
    payload = { ...payload, id: Date.now(), tasks:(payload.tasks||[]).map(t=>String(t).trim()).filter(Boolean) };
    state.templates = [payload, ...state.templates];
    delete state._newTemplate;
  }
  persist(); closeTemplateEditor();
}
...
function patchOutsourceEditor(key,val){
  state.orders = state.orders.map(o => o.id===state.outsourceEditorId ? {...o,[key]:val} : o);
  render();
}
function saveOutsourceEditor(){
...
function patchCalendarQuickEdit(key,val){
  state.orders = state.orders.map(o => o.id===state.calendarQuickEditId ? {...o,[key]: key==="status" ? val : val, judge: key==="status" ? statusToJudge(val) : o.judge } : o);
  render();
}
function saveCalendarQuickEdit(){
  state.orders = state.orders.map(o => {
    if(o.id !== state.calendarQuickEditId) return o;
    const next = { ...o, notice: buildNotice(o), history:[...(o.history||[]),"カレンダーから案件を更新しました"] };
    return next;
  });
  persist(); closeCalendarQuickEdit();
}

function render(){
  document.getElementById("app").innerHTML = `<div class="layout">${renderNav()}<main class="main">${renderPage()}</main>${renderOrderModal()}${renderCreateModal()}${renderStaffEditorModal()}${renderTemplateEditorModal()}${renderOutsourceEditorModal()}${renderCalendarQuickEditModal()}</div>`;
}
render();
